				...
				<div class="d-flex flex-row">
					...
					<div class="col-9 text-center align-self-center">
						<div class="m-l-10 ">
							...
							<p class="mb-0 text-muted"><?= __('admin.total_action_amount') ?></p>
						</div>
					</div>
				</div>
			</div>
...
<div class="row">
					...
					<div class="table-responsive">
						<table class="table table-striped table-sortable wallet-table">
							<thead>
								<tr>
									<th><?= __('admin.id') ?></th>
									<th><?= __('admin.useer') ?></th>
									<th></th>
									<th></th>
									<th><?= __('admin.order') ?></th>
									<th width="200px"><?= __('admin.commission') ?></th>
									<th><?= __('admin.paid_status') ?></th>
									<th><?= __('admin.method') ?></th>
									<th><?= __('admin.date') ?></th>
									<th><?= __('admin.type') ?></th>
									<th><?= __('admin.status') ?></th>
									<th><?= __('admin.wallet') ?></th>
							...
							<tbody>
								...
								<tr class="wallet-id-<?= $value['id'] ?> main-tr <?= $recurring ? 'recurring recurringof-'.$recurring : '' ?> ">
									...
									<td >
										...
										<div class="wallet-popover badge bg-<?= is_need_to_pay($value) ? 'danger' : 'secondary' ?> py-1 pl-2 font-14" toggle="popover"> 
									...
									<td class="text-right">
										...
										<button data-toggle="tooltip" title="<?= __('admin.show_recurring_transition') ?>" class="btn btn-sm btn-primary show-recurring-transition" data-id="<?= $value['id'] ?>"><i class="mdi mdi-plus"></i>
										</button>
										<?php } ?>
										<?php } ?>
									</td>
	...
	$("#modal-confirmstatus").delegate(".close-modal","click",function(){
    	...
    	var id = $(this).data("id");
    	console.log($(".status-change-rdo[name=status_"+ id +"]:not(:checked)"));
    	$(".status-change-rdo[name=status_"+ id +"]:not(:checked)").prop("checked", true)
