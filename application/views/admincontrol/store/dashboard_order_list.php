<?php foreach($orders as $index => $order){ ?>
	<?php if($order['type'] == 'store'){ ?>
		<tr>
			...
			?>
			<td>
				<div class="badge <?= ($order['status'] == 1) ? 'bg-success text-white px-3 py-2 fs-6' : 'bg-warning text-white px-3 py-2 fs-6' ?>">
				    <?= $status[$order['status']] ?>
				</div>
			</td>
			...
			<td>
				...
				if($order['wallet_commission_status'] == 0) {
					?>
					<span class="badge <?php if((int)$order['wallet_status'] > 0){ ?>bg-success text-white px-3 py-2 fs-6<?php }else{ ?>bg-warning text-white px-3 py-2 fs-6<?php } ?>"><?= $wallet_status[(int)$order['wallet_status']] ?></span>
					<?php
			 	} else {
